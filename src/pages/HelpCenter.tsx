    ...
    <div className="space-y-8 max-w-5xl mx-auto relative text-xs leading-normal">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b dark:border-slate-800 pb-4">
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 px-4 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 font-semibold border border-red-500/20 rounded-xl transition-all self-start sm:self-auto cursor-pointer"
        >
      ...
      <div className="flex bg-slate-900/60 border border-slate-850 p-1 rounded-xl gap-0.5">
        <button
          ...
          className={`flex-1 py-2 font-semibold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'faqs' ? 'bg-indigo-650 text-white' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 font-semibold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'guide' ? 'bg-indigo-650 text-white' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 font-semibold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'tutorials' ? 'bg-indigo-650 text-white' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 font-semibold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'ticket' ? 'bg-indigo-650 text-white' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
        ...
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="space-y-3">
            {filteredFaqs.map((faq, index) => (
              <div key={index} className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden text-xs">
                <button
                  onClick={() => toggleFaq(index)}
                  className="w-full p-4 text-left font-semibold text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-850 flex justify-between items-center bg-slate-50 dark:bg-slate-900/30"
                >
                  <span className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded bg-indigo-500/10 text-indigo-400 font-mono text-[9px] uppercase">{faq.category}</span>
                    <span>{faq.q}</span>
                  </span>
                  <span className="text-lg font-bold text-slate-400">{faq.open ? '−' : '+'}</span>
                </button>
                {faq.open && (
                  <div className="p-4 bg-white dark:bg-slate-950 text-slate-400 leading-relaxed border-t dark:border-slate-800">
                    {faq.a}
                  </div>
                )}
              </div>
      ...
      {activeTab === 'guide' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
          {userGuideModules.map((mod, idx) => (
            <div key={idx} className="glass-card p-6 rounded-2xl space-y-3 border dark:border-slate-800">
      ...
      {activeTab === 'tutorials' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
          {videoTutorials.map((tut, idx) => (
            <div key={idx} className="glass-card rounded-2xl overflow-hidden border dark:border-slate-800 flex flex-col group">
              ...
              <div className="p-5 flex-1 space-y-2">
                <h4 className="font-bold text-slate-100 text-xs leading-normal">{tut.title}</h4>
                <p className="text-[10px] text-slate-400 leading-relaxed">{tut.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {activeTab === 'ticket' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in">
          <div className="lg:col-span-2 space-y-4">
            ...
            {ticketSuccess && (
              <div className="flex items-center gap-2 p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-500/20 text-emerald-400 text-xs">
            ...
            <form onSubmit={handleCreateTicket} className="glass-card p-6 rounded-2xl space-y-4">
              ...
              <button
                type="submit"
                className="w-full flex items-center justify-center gap-2 bg-indigo-650 hover:bg-indigo-755 text-white font-semibold py-3 rounded-xl text-xs transition-all shadow-md cursor-pointer"
              >
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            {tickets.length ? (
              <div className="space-y-3">
                {tickets.map(t => (
                  <div key={t.id} className="p-4 bg-slate-900/40 rounded-xl border dark:border-slate-850 space-y-1">
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-slate-200 truncate">{t.subject}</span>
                      <span className="text-[9px] font-bold uppercase text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded">{t.status}</span>
                    </div>
              ...
              <div className="p-6 text-center text-slate-500 border border-dashed dark:border-slate-800 rounded-xl">
      ...
      {reportModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-2xl space-y-4">
            ...
            {issueSuccess && (
              <div className="p-3 bg-emerald-950/40 border border-emerald-500/20 text-emerald-400 rounded-xl text-xs">
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className="p-4 bg-indigo-650 hover:bg-indigo-755 text-white rounded-full shadow-2xl transition-all flex items-center justify-center hover:scale-105 border border-indigo-500/20 cursor-pointer"
      ...
      {chatOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-80 sm:w-96 h-[450px] glass-card rounded-2xl overflow-hidden shadow-2xl flex flex-col border border-indigo-500/20">
          <div className="bg-indigo-650 text-white p-4 flex justify-between items-center">
            ...
            <button 
              ...
              className="text-white hover:text-slate-200 text-xs font-semibold"
            >
              Hide
            </button>
          </div>

          <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-955 text-xs leading-relaxed">
            ...
            {chatLoading && (
              <div className="flex justify-start">
                <div className="p-3 bg-white dark:bg-slate-900 border text-slate-400 rounded-2xl rounded-tl-none animate-pulse">
                  AI is thinking...
                </div>
              </div>
            )}
          ...
          <form onSubmit={handleSendMessage} className="p-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex gap-2">
            ...
            <button
              type="submit"
              className="p-2 bg-indigo-650 hover:bg-indigo-755 text-white rounded-lg transition-all shrink-0 cursor-pointer"
            >
