...
export const Portfolio: React.FC = () => {
  ...
  return (
    <div className="space-y-8 text-xs leading-normal">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 self-start sm:self-auto">
          <button
            ...
            className="flex items-center gap-1.5 bg-indigo-650 hover:bg-indigo-755 text-white font-semibold py-2 px-3.5 rounded-xl transition-all shadow-md cursor-pointer"
          >
      ...
      <div className="flex bg-slate-100 dark:bg-slate-950 p-1 rounded-xl gap-1 text-[11px] border border-slate-200 dark:border-slate-800">
        <button
          ...
          className={`flex-1 py-2 font-semibold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'preview' ? 'bg-indigo-650 text-white shadow-md' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 font-semibold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'editor' ? 'bg-indigo-650 text-white shadow-md' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100'
          }`}
        >
        ...
        <button
          onClick={() => setActiveTab('wizard')}
          className={`flex-1 py-2 font-semibold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'wizard' ? 'bg-indigo-650 text-white shadow-md' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100'
          }`}
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 glass-card p-6 rounded-2xl space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <span className="bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-lg px-2.5 py-2 text-[10px] text-slate-500 font-mono flex items-center shrink-0">
                  /portfolio/
                </span>
                <input
                  type="text"
            ...
            <div className="space-y-2">
              ...
              >
                <option value="sleek">Sleek Corporate (Indigo & Dark Slate)</option>
            ...
            <div className="space-y-3 pt-2">
              {portfolio.is_published && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-indigo-650 hover:bg-indigo-755 text-white font-semibold py-2.5 rounded-xl text-xs transition-all shadow-lg"
                >
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setActiveTab('editor')}
                  className="flex-1 py-2 border border-slate-300 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-xs font-semibold text-center"
                >
                  Edit Sections
                </button>
                <button
                  onClick={handleDeletePortfolio}
                  className="px-3 py-2 border border-red-500/20 hover:bg-red-500/10 text-red-500 rounded-xl text-xs font-semibold"
            ...
            {/* QR Code section */}
            {portfolio.is_published && (
              <div className="pt-4 border-t dark:border-slate-800 flex flex-col items-center gap-2">
                <span className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1">
                  <QrCode className="w-3.5 h-3.5 text-indigo-500" /> Recruiter QR Code
                </span>
                <img
          ...
          <div className="lg:col-span-8 space-y-4">
            <div className="flex justify-between items-center text-xs font-semibold text-slate-400">
              ...
              <span className="text-indigo-500 font-bold uppercase text-[10px]">Theme: {portfolio.theme}</span>
            </div>
            ...
            <div className={`rounded-2xl border overflow-hidden shadow-2xl p-8 min-h-[500px] flex flex-col justify-between transition-all ${
              ...
              <header className="flex justify-between items-start border-b pb-6 dark:border-slate-800 border-slate-200">
                ...
                <span className={`text-[9px] uppercase font-bold border px-2.5 py-1 rounded-full ${
                  portfolio.theme === 'cyberpunk' ? 'border-cyan-500 text-cyan-400' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'
                }`}>
                  Open to Roles
                </span>
              </header>
              ...
              <main className="my-6 space-y-6 flex-1 text-xs">
                ...
                <div className="space-y-1.5">
                  <h3 className={`font-bold text-xs uppercase tracking-wider ${portfolio.theme === 'cyberpunk' ? 'text-pink-500' : 'text-indigo-500'}`}>About Me</h3>
                ...
                {/* Core Skills */}
                <div className="space-y-2">
                  <h3 className={`font-bold text-xs uppercase tracking-wider ${portfolio.theme === 'cyberpunk' ? 'text-pink-500' : 'text-indigo-500'}`}>Core Skills</h3>
                ...
                {portfolio.selected_projects.length > 0 && (
                  <div className="space-y-3 pt-2">
                    <h3 className={`font-bold text-xs uppercase tracking-wider ${portfolio.theme === 'cyberpunk' ? 'text-pink-500' : 'text-indigo-500'}`}>Featured Projects</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {portfolio.selected_projects.map((p, idx) => (
                        <div key={idx} className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 space-y-1.5">
                          ...
                          <span className="text-[9px] font-mono text-indigo-500 block pt-1">{p.tech}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {portfolio.selected_education.length > 0 && (
                  <div className="space-y-2 pt-2">
                    <h3 className={`font-bold text-xs uppercase tracking-wider ${portfolio.theme === 'cyberpunk' ? 'text-pink-500' : 'text-indigo-500'}`}>Education</h3>
                    {portfolio.selected_education.map((e, idx) => (
                      <div key={idx} className="flex justify-between items-center text-xs">
                        ...
                        <span className="text-[10px] font-semibold text-indigo-400">{e.year} • GPA {e.gpa}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {/* Footer Links */}
              <footer className="flex justify-between items-center text-[10px] text-slate-500 border-t pt-4 dark:border-slate-800 border-slate-200">
      ...
      {activeTab === 'editor' && (
        <div className="glass-card p-6 rounded-2xl space-y-6 max-w-4xl mx-auto">
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider border-b dark:border-slate-800 pb-3">
          ...
          <div className="space-y-3 pt-2 border-t dark:border-slate-800">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={addSkill}
                className="px-4 py-2 bg-indigo-650 text-white font-bold rounded-lg text-xs"
              >
                Add Skill
              </button>
            </div>
            <div className="flex flex-wrap gap-1.5 pt-2">
              {portfolio.selected_skills.map((s, idx) => (
                <span key={idx} className="px-2.5 py-1 rounded bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex items-center gap-1.5 text-[10px]">
                  <span>{s}</span>
                  <button onClick={() => removeSkill(idx)} className="text-red-400 hover:text-red-500 font-bold">×</button>
                </span>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3 pt-4 border-t dark:border-slate-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <button
                onClick={addProject}
                className="py-2 bg-indigo-650 text-white font-bold rounded-lg text-xs"
              >
                Add Project Card
              </button>
            </div>
            ...
            <div className="space-y-2 pt-2">
              {portfolio.selected_projects.map((p, idx) => (
                <div key={idx} className="p-3 bg-slate-50 dark:bg-slate-950/40 rounded-xl border border-slate-200 dark:border-slate-800 flex justify-between items-center text-xs">
                  <div>
                    <span className="font-bold text-slate-800 dark:text-slate-100">{p.title}</span>
                    <span className="text-[10px] text-indigo-500 font-mono ml-2">({p.tech})</span>
                  </div>
                  <button onClick={() => removeProject(idx)} className="text-red-400 hover:text-red-500 font-bold text-xs">Remove</button>
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === 'wizard' && (
        <div className="glass-card p-8 rounded-2xl space-y-6 max-w-2xl mx-auto">
          <div className="flex justify-between items-center border-b dark:border-slate-800 pb-3">
            <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">
              Portfolio Creation Wizard (Step {wizardStep} of 4)
            </h3>
            <span className="text-[10px] text-indigo-500 font-bold uppercase">Automated Import</span>
          </div>
          ...
          {wizardStep === 2 && (
            <div className="space-y-4 text-xs">
              ...
              <div className="flex flex-wrap gap-1.5">
                {portfolio.selected_skills.map((s, idx) => (
                  <span key={idx} className="px-2 py-1 bg-indigo-500/10 border border-indigo-500/20 text-indigo-500 rounded text-[10px]">
                    {s}
                  </span>
                ))}
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {['sleek', 'minimal', 'cyberpunk'].map((t) => (
                  <button
                    ...
                    className={`p-4 rounded-xl border text-center font-bold uppercase text-[10px] capitalize transition-all ${
                      portfolio.theme === t ? 'border-indigo-500 bg-indigo-500/10 text-indigo-500' : 'border-slate-300 dark:border-slate-800'
                    }`}
                  >
                    {t}
                  </button>
          ...
          <div className="flex justify-between pt-4 border-t dark:border-slate-800 text-xs">
            <button
              onClick={() => setWizardStep(Math.max(1, wizardStep - 1))}
              disabled={wizardStep === 1}
              className="px-4 py-2 border border-slate-300 dark:border-slate-800 rounded-xl font-semibold disabled:opacity-40"
            >
              Previous
            </button>
            ...
            {wizardStep < 4 ? (
              <button
                ...
                className="px-4 py-2 bg-indigo-650 text-white rounded-xl font-semibold"
              >
                Next Step
              </button>
            ) : (
